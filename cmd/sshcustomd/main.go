...
func runCmd() {
	...
	httpSrv := &http.Server{
		Addr:         "127.0.0.1:9190",
		Handler:      buildHTTPMux(atomicCfg, *workDir, *cfgPath, st, &sshClient),
...
func buildHTTPMux(
	...
	// enriched /status, config patch, and static webroot) for the v1.0.1 WebUI.
	// These override/extend the basic handlers above where needed.
	registerExtendedAPI(mux, workDir, cfgPath, st, clientPtr)

	// Wrap everything with CORS so the KSU-manager WebView (different origin)
	// and WebUI-X can call the API without cross-origin errors.
	return withCORS(mux)
}
